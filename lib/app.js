...
app.get('/yelp', async (req, res) => {
  try {
    const URL = `https://api.yelp.com/v3/businesses/search?latitude=${req.query.lat}e&longitude=${req.query.lng}`;

    const response = await request.get(URL).set({ 'Authorization': `Bearer ${process.env.WEATHER_KEY}` });
